<?php
	...
	if($active == 1 ){
		...
		if($dbh = $_db->get('Event_Req', array('Operation_Date','>=',$_GET["start"],'Operation_Date','<=',$_GET["end"]))){    //get data from event table. No where statement required
				if($dbh->counts() > 0){
					$i = 0;
					...
					{	
						...
						//	$details = $hospitalname . ", " . $doctorname;
							...
							$singleEvent = array(
							'id' =>  $key->Id,	
							'title' => $organiser . "\r\n" . $hospitalname . " - " . $doctorname . "\r\n" . "Equipment: " . $equipmentrequired . "\r\n" . "Rep Attend: " . $key->Rep_Attend . " / " . "Consignment: " . $consign . " / Status: " . ucfirst($key->Status),            
