<?php
	...
	if($active == 1 ){
		...
		if($dbh = $_db->get('Event_Req', array('Organiser','=',$userid,'Operation_Date','>=',$_GET["start"],'Operation_Date','<=',$_GET["end"],'Status','=','delivered'))){
		...
		if($dbh->counts() > 0){
			...
			{	$delivered = 0;
				...
				//if($delivered == 1 && $collect == 0 && $tobecollect == 0)
				//{
					//$obj = new stdClass();
