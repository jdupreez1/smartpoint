<?php
	...
	if($active == 1 ){
		...
		if($dbh = $_db->get('Event_Req', array('Consignment','=',0,'Operation_Date','>=',$_GET["start"],'Operation_Date','<=',$_GET["end"],'Status','=','used'))){    //get data from event table. No where statement required
				if($dbh->counts() > 0){
					$i = 0;
