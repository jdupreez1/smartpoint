<?php
	...
	if($active == 1 ){
		...
		if($dbh = $_db->get('Event_Req', array('Organiser','=',$userid,'Consignment','=',1,'Operation_Date','>=',$_GET["start"],'Operation_Date','<=',$_GET["end"],'Status','=','used'))){    //get data from event table. No where statement required
				if($dbh->counts() > 0){
					$i = 0;
					...
					{	
						$tobecollect = 1;
						$collect = 0;
						/*if($dbh2 = $_db->get('Event_Usage', array('Event_Req_Id','=',$key->Id,'Qty_Used','>',0,'Qty_Used','<>','Qty_Refilled'))){    //get data from event table. No where statement required
							if($dbh2->counts() > 0){
