<?php
	...
	if($active == 1 ){   //get all the events acked by this driver

		if($dbh = $_db->get('Event_Req', array('Driver_Ack','=',$userid,'Operation_Date','>=',$_GET["start"],'Operation_Date','<=',$_GET["end"],'Status','=','acked'))){    //get data from event table. No where statement required
				if($dbh->counts() > 0){
					$i = 0;
					...
					{	$delivered = 0;
						...
						*/
						...
						if($delivered == 0 && $collect == 0 && $tobecollect == 0)
						{
							//$obj = new stdClass();
